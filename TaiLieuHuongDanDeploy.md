...
1. Truy cập Azure Portal và chọn **Create a resource** → Chọn **SQL Database**.
2. Cấu hình các thông tin cơ bản:
   ...
   * **Compute + storage:** Để tiết kiệm chi phí chọn **Looking for production** → Đổi thành **Serverless** hoặc gói **DTU-based (Basic)** để tránh phát sinh chi phí lớn.
...
1. Vào tài nguyên SQL Server vừa tạo → ngay tại tab **Overview** nhìn lên menu trên cùng → chọn **Set server firewall**.
...
1. Trên Azure Portal, chọn **Create a resource** → Chọn **Web App (App Service)**.
...
1. Vào Web App vừa tạo → Menu trái chọn **Environment variables**.
2. Tại tab **App settings**, nhấn **Add** để tạo biến mới:
   * *Tuỳ vào số lượng biến trong file cấu hình (appsettings.json)*
   * **Name:** `ConnectionStrings__DefaultConnection` (hoặc tên tương ứng trong file cấu hình của bạn).
   * **Value:** Điền chuỗi kết nối ADO.NET của Azure SQL (quay lại trang Azure SQL và ấn vào DB tại menu bên trái chọn **Settings** → **Connection strings** → copy chuỗi kết nối).
...
1. Trong Web App, chọn **Deployment** → **Deployment Center** ở menu bên trái.
...
1. **Framework Preset:** Vercel sẽ tự nhận diện framework bạn dùng (React, Next.js, Vite, v.v.). Nếu nhận diện đúng, giữ nguyên, nếu không thì chọn vào framework dự án đang sử dụng.
...
4. **Lưu ý quan trọng từ Azure:** URL nhập vào không được chứa dấu gạch chéo ở cuối.
