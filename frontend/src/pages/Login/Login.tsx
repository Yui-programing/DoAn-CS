...
import { Music, Eye, EyeOff, Lock, Mail, ArrowRight } from 'lucide-react';
...
export const Login = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Tạm thời điều hướng về trang chủ khi nhấn Login để demo
    navigate('/');
  };
    ...
    <div className="w-screen min-h-screen bg-gradient-to-br from-slate-950 via-zinc-950 to-emerald-950/80 flex items-center justify-center p-4 font-sans text-slate-100">
      <div className="max-w-md w-full p-8 bg-zinc-900/60 backdrop-blur-md border border-zinc-800 rounded-3xl shadow-2xl flex flex-col items-center cursor-default transition-all duration-300 hover:border-zinc-700/60">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          <button 
            type="submit"
            className="w-full py-3 bg-green-500 hover:bg-green-400 text-black font-bold text-sm rounded-xl transition-all duration-200 hover:scale-[1.01] active:scale-95 shadow-md flex items-center justify-center gap-1.5 mt-2"
          >
            <span>Đăng nhập</span>
